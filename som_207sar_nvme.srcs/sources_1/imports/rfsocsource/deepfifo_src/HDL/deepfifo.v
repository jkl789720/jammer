module deepfifo
  #(
   ...
   parameter log2_ram_size_addr = 32,
   parameter log2_word_width = 9,
   parameter log2_fifo_words = 9,
   parameter log2_burst_words = 8,//缺省情况下是4,kc705的1GBDDR3,row地址是14位
...
//   localparam log2_words_in_ram = log2_ram_size_addr - log2_word_addr_size;
 //  localparam log2_bursts_in_ram = log2_words_in_ram - log2_burst_words;
  ...
  input [word_width-1:0]     fifo_pre_dout;
   ...
   output reg [log2_bursts_in_ram:0]   bursts_stored;  //log2_bursts_in_ram
  output reg  do_from_ram_burst;

   reg 				resetn_async;
   reg [2:0] 			resetn_guard;

   (* keep = "true" *)reg 				bypass_mode;

    (* keep = "true" *)reg 				fifo_pre_fetch;
    (* keep = "true" *)reg 				fifo_pre_valid;

   (* keep = "true" *)reg 				beat_to_ram, beat_from_ram;
   (* keep = "true" *)reg 				do_to_ram_burst;
   (* keep = "true" *)reg [log2_burst_words:0] 	to_ram_increase, from_ram_decrease;
   ...
   (* keep = "true" *)reg [log2_bursts_in_ram:0] 	bursts_allocated;
   (* keep = "true" *)reg [log2_bursts_in_ram-1:0] to_ram_burst_pos, from_ram_burst_pos;
   (* keep = "true" *)reg [log2_fifo_words:0] 	min_pre_rd_count;
   (* keep = "true" *)reg [log2_fifo_words+1:0] 	max_post_wr_count;
